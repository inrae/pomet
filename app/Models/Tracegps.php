...
class Tracegps extends PpciModel
{
        ...
        if (strlen($filename) > 0) {
            if (filesize($filename) > 10) {
                ...
                if (!$this->dom->load($filename)) {
                    throw new PpciException("Impossible de lire le fichier $filename");
                }
                ...
                throw new PpciException("File is empty or too small");
            }
        } else {
            throw new PpciException("Filename is empty");
        }
        return $this->traces;
    }
    ...
    public function importCsv(int $trait_id, string $filename, string $separator = ",")
    {
        if (!file_exists($filename)) {
            throw new PpciException("Le fichier n'a pas été téléchargé");
        }
        $handle = fopen($filename, 'r');
        if (!$handle) {
            throw new PpciException("Le fichier n'a pas pu être ouvert par le serveur");
        }
        /**
        ...
        if (count($header) != 3) {
            throw new PpciException("Le fichier ne contient pas les colonnes nécessaires pour l'importation : time, lat, lon");
        }
        $data = array();
        $line = "st_makeline(array[";
        $comma = "";
