...
/**
 * Instanciate a translateId class in $_SESSION
 *
 * @param string $name
 * @return void
 */
function translateIdInstanciate($name)
{
	if (!isset($_SESSION[$name])) {
		$tikey = [
			"ti_campagne" => "campagne_id",
			"ti_trait" => "trait_id",
			"ti_echantillon" => "ech_id",
			"ti_individu" => "ind_id"
		];
		$_SESSION[$name] = new TranslateId($tikey[$name]);
	}
}
